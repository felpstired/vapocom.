...
INSERT INTO `tbartesvend` (`idartesvend`,`idvendedor`,`imgarte`,`titulo`,`descricao`,`valor`,`ativo`,`copias`) VALUES 
 ...
 (4,4,'moss/arte1.png','OC','Um dos muitos e muitos redraws do meu OC/Icon eba eba eba :]',46,'A',78),
...
INSERT INTO `tbcarrinho` (`idcarrinho`,`idusuario`,`idartesvend`,`ativo`,`qtdd`,`cadastro`,`alteracao`) VALUES 
 ...
 (46,4,3,'D',1,'2023-10-05 17:39:27','2023-10-05 17:52:12');
...
--
-- Definition of table `tbpedidoscart`
--

DROP TABLE IF EXISTS `tbpedidoscart`;
CREATE TABLE `tbpedidoscart` (
  `idpedidoscart` int(10) unsigned NOT NULL AUTO_INCREMENT,
  ...
  `alteracao` timestamp NOT NULL DEFAULT current_timestamp() ON UPDATE current_timestamp(),
  `idpagamento` int(10) unsigned NOT NULL,
  PRIMARY KEY (`idpedidoscart`,`idcarrinho`,`idpagamento`) USING BTREE,
  KEY `FK_tbpedidoscart_tbcarrinho` (`idcarrinho`) USING BTREE,
  KEY `FK_tbpedidoscart_tbpagamento` (`idpagamento`),
  CONSTRAINT `FK_tbpedidoscart_tbcarrinho` FOREIGN KEY (`idcarrinho`) REFERENCES `tbcarrinho` (`idcarrinho`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `FK_tbpedidoscart_tbpagamento` FOREIGN KEY (`idpagamento`) REFERENCES `tbpagamento` (`idpagamento`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_general_ci;

--
-- Dumping data for table `tbpedidoscart`
--

/*!40000 ALTER TABLE `tbpedidoscart` DISABLE KEYS */;
/*!40000 ALTER TABLE `tbpedidoscart` ENABLE KEYS */;
